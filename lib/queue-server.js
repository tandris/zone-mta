...
class QueueServer {
            ...
            client.onData = (data, next) => {
                ...
                switch (data.cmd) {
                    ...
                    case 'RELEASE':
                        ...
                        return this.releaseDelivery(client.zone, client.id, data, (err, response, meta) => {
                            if (!client) {
                                // client already errored or closed
                                return;
                            }
                            ...
                            plugins.handler.runHooks('naga:sent', [client.zone, data, meta], () => {});
                    ...
                    case 'REMOVE':
                        this.queue.removeMessage(data.id, err => {
                            ...
                            this.queue.getMeta(data.id, (err, meta) => {
                                plugins.handler.runHooks('naga:remove', [client.zone, data, meta], () => {});
                            });
                            if (err) {
                                return client.send({
                                    req: data.req,
    ...
    releaseDelivery(zone, lockOwner, req, callback) {
        this.queue.getDelivery(req.id, req.seq, (err, delivery) => {
            ...
            this.queue.getMeta(req.id, (err, meta) => {
                zone.releaseDelivery(delivery, err => {
                    if (err) {
                        return callback(err);
                    }
                    return callback(null, delivery.id + '.' + delivery.seq, meta);
                });
            });
        });
    }
    ...
    deferDelivery(zone, lockOwner, req, callback) {
        this.queue.getDelivery(req.id, req.seq, (err, delivery) => {
            ...
            this.queue.getMeta(delivery.id, (err, meta) => {
                plugins.handler.runHooks('naga:defer', [zone, delivery, meta], () => {});
            });
